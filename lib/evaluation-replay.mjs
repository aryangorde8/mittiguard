...
async function completeReplayTask(store, caseId, task) {
  if (task.ownerRole !== "FIELD_CAPTURE") {
    return store.recordTaskEvidence(caseId, task.id, "Synthetic evidence received for replay.");
  }
  ...
  return store.recordFieldCaptureEvidence(issued.token, {
    observation: "Current requested evidence captured for deterministic replay.",
    imageMetadata: { mediaType: "image/jpeg", bytes: 1234, sha256: "F".repeat(64) }
  });
}
...
async function replayRelayIntegrity(directory) {
  ...
  for (const task of record.relay.tasks) {
    updated = await completeReplayTask(localStore, record.id, task);
  }
