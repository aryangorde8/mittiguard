...
// V2 binds ledger identity and coverage metadata into every sealed event. A
// V1 demo ledger must be reset rather than silently treated as equivalently
// anchored evidence.
...
export function verifyAuditLedger(data, { caseId = null } = {}) {
  ...
  for (const [index, entry] of ledger.entries.entries()) {
    ...
    const ledgerIdentityValid = entry.ledgerId === ledger.ledgerId
      && entry.ledgerVersion === ledger.version
      && entry.coverage === ledger.coverage
      && entry.algorithm === (entry.sealed === true ? SEALED_ALGORITHM : DEVELOPMENT_ALGORITHM);
    ...
    if (!sequenceValid || !ledgerIdentityValid || !previousValid || !hashValid || !mirrorValid) {
      valid = false;
