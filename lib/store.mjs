...
function fieldCaptureContext(record, task, capability) {
  const captureRequirement = fieldCaptureRequirement(task);
  return {
    ...
    task: {
      ...
      dueAt: task.dueAt,
      captureRequirement,
      imageRequired: captureRequirement !== "NONE"
    },
...
function fieldCaptureRequirement(task = {}) {
  // Persist the requirement on newly-created tasks, while deriving it for
  // previously-created demo records so an older ledger cannot weaken the
  // mobile capture boundary after an app upgrade.
  if (["FIELD_PHOTO", "SOIL_CARD_IMAGE", "NONE"].includes(task.captureRequirement)) {
    return task.captureRequirement;
  }
  const evidence = String(task.evidence || task.title || "");
  if (/photo/i.test(evidence)) return "FIELD_PHOTO";
  if (/soil test|soil health/i.test(evidence)) return "SOIL_CARD_IMAGE";
  return "NONE";
}
...
function evidencePayload(record, invoiceId) {
  return {
    ...
    tasks: (record.relay?.tasks || []).map((task) => ({
      ...
      captureRequirement: fieldCaptureRequirement(task),
      status: task.status,
...
function buildTasks(record, createdAt) {
  return (record.requiredEvidence || []).map((requiredEvidence, index) => ({
    ...
    captureRequirement: /photo/i.test(requiredEvidence)
      ? "FIELD_PHOTO"
      : /soil test|soil health/i.test(requiredEvidence)
        ? "SOIL_CARD_IMAGE"
        : "NONE",
    status: "REQUESTED",
...
export class MittiStore {
  ...
  async recordFieldCaptureEvidence(rawToken, input = {}) {
    return this.update((data) => {
      ...
      const captureRequirement = fieldCaptureRequirement(task);
      if (captureRequirement !== "NONE" && !image) {
        const label = captureRequirement === "SOIL_CARD_IMAGE" ? "Soil Health Card / test image" : "field image";
        throw requestError(`This task requires a ${label} before it can be submitted.`);
      }
      ...
      // A photographed Soil Health Card is evidence, but it is not a field
      // image. Keep this signal precise so the Decision Room never claims a
      // crop photograph was attached when only a document was captured.
      record.photoAttached = Boolean(record.photoAttached || (captureRequirement === "FIELD_PHOTO" && image));
  ...
  async recordTaskEvidence(id, taskId, note) {
    return this.update((data) => {
      ...
      if (task.ownerRole === "FIELD_CAPTURE") {
        throw requestError("This Field Capture task must be completed through its time-bound Field Capture link.");
      }
