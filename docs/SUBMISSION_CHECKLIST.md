...
- [ ] Run `npm run check` and `npm run smoke:model`; save a terminal screenshot showing both pass. On the deployed server, also run `npm run eval:intake:nova:required -- --json` and retain the unedited passing report as a synthetic intake-contract evaluation.
- [ ] Before recording, use **Load clean jury demo** and confirm the reset, then use **Run bypass proof**. It resets the shared synthetic jury ledger for that deployment, restores one curated field history, and exercises the real server-side Evidence Debt path.
- [ ] Record the under-three-minute demo using [DEMO.md](DEMO.md). Keep the audio factual: Codex/GPT-5.6 accelerated the build; Amazon Nova Pro is the live runtime evidence-summary provider. If the draft labels itself as a deterministic fallback, fix deployment and re-record rather than presenting it as Nova.
